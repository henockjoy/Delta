...
async def Lucy_start():
    ...
    try:
        ...
        restart_text = script.RESTART_TXT.format(temp.B_LINK, today, time)
        restart_channels = [LOG_CHANNEL]
        if AUTH_CHANNEL and AUTH_CHANNEL not in restart_channels:
            restart_channels.append(AUTH_CHANNEL)
        for channel_id in restart_channels:
            await Codeflix.send_message(chat_id=channel_id, text=restart_text)
